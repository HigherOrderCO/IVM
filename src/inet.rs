...
impl INet {
			...
			for port_idx in 0 .. node.ports.len() {
				...
				if src < dst {
					let mut connector = |node_port: NodePort| {
						...
						let agent_port_names =
							node_idx_to_agent_port_names.entry(node_port.node_idx).or_insert_with(|| {
								(0 .. node.ports.len() - 1).map(|_| new_port_name()).collect()
							});
	...
	pub fn add_connections<'a>(
		...
		let mut ports_to_link_later = Vec::<[MaybeLinkedPort; 2]>::new();
		{
			...
			ports_to_link_later.extend(
				external_links.into_iter().map(|(port_name, node_port)| [Err(port_name), Ok(node_port)]),
			);
		}
